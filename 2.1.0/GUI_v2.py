...
class GUI(QMainWindow):
    ...
    def initGUI(self):
        ...
        # Create frames for special widgets
        if self.device_type == "widget":
            ...
            grid.addWidget(self.open_camera_button, 1, 0)
            ...
            grid.addWidget(self.capture_button, 2, 0)

            # Switch mode
            self.switch_button = QCheckBox()
            self.switch_button.setStyleSheet("""
                QCheckBox {
                        background-color: white; /* Background color of the switch track */
                        border-radius: 15px; /* Round the corners of the track */
                        width: 30px; /* Width of the entire switch */
                        height: 30px; /* Height of the entire switch */
                }
                QCheckBox::indicator {
                    background-color: red; /* Initial color of the switch handle */
                    border-radius: 10px; /* Round the corners of the handle */
                    width: 30px; /* Width of the handle */
                    height: 30px; /* Height of the handle */
                }
                QCheckBox::indicator:checked {
                    background-color: green; /* Color of the switch handle when toggled on */
                    border-radius: 10px; /* Round the corners of the handle */
                    margin-left: 37.5px; /* Move the handle to the right side of the track */
                }
            """)
            self.switch_button.stateChanged.connect(self.toggle_switch)
            grid.addWidget(self.switch_button, 0, 1, 1, 1)
            ...
            grid.addWidget(self.zoom_in_button, 1, 1)
            ...
            grid.addWidget(self.zoom_out_button, 2, 1)
            ...
            # Align Polygon
            self.align_button = QPushButton('Align Flake')
        ...
        # Create frames for devices
            ...
            if self.device_channel is not None:  # For KIM devices
                device_label = QLabel(
                    ...
                    self)
            font = QFont("Arial")
            font.setPointSize(8)  # Set the font size to 14 points
            ...
            # grid.addWidget(self.calibration_button, 5, 0, 1, 2)
            ...
            if self.device_channel is None:  # For CC devices
                ...
                self.step_entry = QLineEdit(self.choices_frame)
                self.step_entry.setValidator(QDoubleValidator())
                self.step_entry.setText("1000")
                choices_grid.addWidget(self.step_entry, 0, 6)

                self.rate_label = QLabel("Rate", self.choices_frame)
                self.rate_label.setFont(label_font)
                self.rate_label.setStyleSheet("background-color: lightgray;")
                choices_grid.addWidget(self.rate_label, 1, 5)

                self.rate_entry = QLineEdit(self.choices_frame)
                self.rate_entry.setValidator(QDoubleValidator())
                self.rate_entry.setText("1000")
                choices_grid.addWidget(self.rate_entry, 1, 6)

                self.acceleration_label = QLabel("Acceleration", self.choices_frame)
                self.acceleration_label.setFont(label_font)
                self.acceleration_label.setStyleSheet("background-color: lightgray;")
                choices_grid.addWidget(self.acceleration_label, 2, 5)

                self.acceleration_entry = QLineEdit(self.choices_frame)
                self.acceleration_entry.setValidator(QDoubleValidator())
                self.acceleration_entry.setText("10000")
                choices_grid.addWidget(self.acceleration_entry, 2, 6)

                self.voltage_label = QLabel("Voltage", self.choices_frame)
                self.voltage_label.setFont(label_font)
                self.voltage_label.setStyleSheet("background-color: lightgray;")
                choices_grid.addWidget(self.voltage_label, 0, 7)

                self.voltage_entry = QLineEdit(self.choices_frame)
                self.voltage_entry.setValidator(QDoubleValidator())
                self.voltage_entry.setText("1000")
                choices_grid.addWidget(self.voltage_entry, 0, 8)

                self.jogmode_label = QLabel("Mode", self.choices_frame)
                self.jogmode_label.setFont(label_font)
                self.jogmode_label.setStyleSheet("background-color: lightgray;")
                choices_grid.addWidget(self.jogmode_label, 1, 7)

                self.jogmode_entry = QLineEdit(self.choices_frame)
                self.jogmode_entry.setValidator(QDoubleValidator())
                self.jogmode_entry.setText("2")
                choices_grid.addWidget(self.jogmode_entry, 1, 8)
    ...
    def toggle_switch(self, state):
        if state == Qt.Checked:
            self.switch_on_function()
            print("Start designing...")
        else:
            self.switch_off_function()
            print("Continue tracking...")

    def switch_on_function(self):
        globals.drawing_mode = True

    def switch_off_function(self):
        globals.drawing_mode = False
    ...
    def fivefold(self):
        ...
        print(f"Conversion size: {parameters['Camera']['Rescale']}")
    ...
    def retrieve_polygon(self):
        print("Retrieve button pressed. Getting flake profile...")
        polygon_profile = get_polygon()  # Replace this with your actual code.
        print(f"Flake profile: {polygon_profile}")

    def calibrate(self):
        global parameters

        print("Calibrate button pressed. Getting shift vector...")
        ...
        print(f"Shift_vector: {shift_vector}")
        ...
        for _ in range(num_small_steps_x):
            print(move_CCdevice(self.device_profile["servo"][1], "servo", None, direction_x, small_step_size_x, velocity_x))
        ...
        for _ in range(num_small_steps_y):
            print(move_CCdevice(self.device_profile["servo"][2], "servo", None, direction_y, small_step_size_y, velocity_y))
    ...
    def align(self):
        ...
        print("Align button pressed. Getting shift vector...")
        ...
        for _ in range(num_small_steps_r):
            print(move_CCdevice(self.device_profile["servo"][3], "servo", None, direction_r, -small_angle_size, velocity_r))

        # Handle any remainder after dividing by small_step_size
        remainder_r = int(rotate_vector_device % small_angle_size)
        if remainder_r > 0:
            print(move_CCdevice(self.device_profile["servo"][3], "servo", None, direction_r, -remainder_r, velocity_r))
    ...
    def move_KIMdevice(self, direction):
        ...
        mode = int(self.jogmode_entry.text())
    ...
    def apply_parameters(self):
        ...
        if self.device_channel is None:
            ...
            try:
                parameters[self.device_name]["KIMstep"] = float(self.step_entry.text())
                parameters[self.device_name]["KIMrate"] = float(self.rate_entry.text())
                parameters[self.device_name]["KIMacceleration"] = float(self.acceleration_entry.text())
                parameters[self.device_name]["KIMjogmode"] = float(self.jogmode_entry.text())
                parameters[self.device_name]["KIMvoltage"] = float(self.voltage_entry.text())
            except ValueError:
                error_dialog = QMessageBox()
                error_dialog.setWindowTitle("Invalid input")
                error_dialog.setText("Please enter valid numbers.")
