...
def disconnect_device(device_serial_num, device_type, device_channel):
    if device_type == 'inertial':
        call_lib(device_type, "EnableChannel", device_serial_num, c_int(int(device_channel)))
...
def is_connected(device_serial_num, device_type, device_channel):
    ...
    if device_type == 'inertial':
        return call_lib(device_type, "Open", device_serial_num, c_int(int(device_channel))) == 0 and call_lib(device_type, "EnableChannel", device_serial_num, c_int(int(device_channel))) == 0, "No devices connected."