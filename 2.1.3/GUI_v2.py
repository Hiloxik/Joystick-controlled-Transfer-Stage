...
from camera_v2 import main as camera_main, get_polygon, get_shift, get_angle, zoom_in_camera, zoom_out_camera, capture_frame
from maths import *
...
class GUI(QMainWindow):
    ...
    def calibrate(self):
        ...
        device_steps_per_view_x = parameters["Camera"]["Rescale"][0] / (parameters["Sample Stage X-Axis"]["CCstep"] / 500)  # full device x movement
        device_steps_per_view_y = parameters["Camera"]["Rescale"][1] / (parameters["Sample Stage Y-Axis"]["CCstep"] / 500) # full device y movement
    ...
    def align(self):
        ...
        if self.align_button.text() == "Align Flake":
            ...
            conversion_factor_x = device_steps_per_view_x / camera_pixels_per_view_x
            conversion_factor_y = device_steps_per_view_y / camera_pixels_per_view_y
            conversion_factor_rotate = 41109.17 / (parameters["Sample Stage Rotator"]["CCstep"] / 1000)
            ...
            # Divide rotational calibration into small steps
            for _ in range(int(abs(rotate_angle))):
                ...
                print(move_CCdevice(self.device_profile["servo"][3], "servo", None, direction_r,
                                    ...
                                    velocity_r))
                time.sleep(0.5)
                print(move_CCdevice(self.device_profile["servo"][1], "servo", None, direction_calx, -abs(int(cal_x)),
                                    velocity_calx))
                time.sleep(0.5)
                print(move_CCdevice(self.device_profile["servo"][2], "servo", None, direction_caly, -abs(int(cal_y)),
                                    velocity_caly))
                time.sleep(0.5)
    ...
    def move_KIMdevice(self, direction):
        global parameters
        step = int(parameters[self.device_name]["KIMstep"])
        rate = int(parameters[self.device_name]["KIMrate"])
        acceleration = int(parameters[self.device_name]["KIMacceleration"])
    ...
    def drive_KIMdevice(self, direction):
        # velocity =
        # Start driving the device in a new thread
        voltage = int(parameters[self.device_name]["KIMvoltage"])
        rate = int(parameters[self.device_name]["KIMrate"])
        acceleration = int(parameters[self.device_name]["KIMacceleration"])
