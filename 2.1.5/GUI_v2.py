...
from camera_v2 import main as camera_main, get_polygon, get_shift, get_angle, zoom_in_camera, zoom_out_camera, \
    ...
    def align(self):
        ...
        if self.align_button.text() == "Align Flake":
            ...
            if globals.origin_coordinate is not None:
                ...
                # Start calibration
                cal_x = (align_radius * (np.cos(-rotate_angle * np.pi / 180 + align_theta) - np.cos(
                    align_theta))) * conversion_factor_x
                cal_y = (align_radius * (np.sin(-rotate_angle * np.pi / 180 + align_theta) - np.sin(
            ...
            def move_calx_device():
                ...
                move_CCdevice(self.device_profile["servo"][1], "servo", None, direction_calx, -abs(int(cal_x)),
                              velocity_calx)
                ppx = get_current_position(self.device_profile["servo"][1], "servo", None)
            ...
            def move_caly_device():
                ...
                move_CCdevice(self.device_profile["servo"][2], "servo", None, direction_caly, abs(int(cal_y)),
                              velocity_caly)
                ppy = get_current_position(self.device_profile["servo"][2], "servo", None)
            ...
            # Start shift
            origin_position_x = get_current_position(self.device_profile["servo"][1], "servo", None)
            origin_position_y = get_current_position(self.device_profile["servo"][2], "servo", None)

            shift_vector_device = (
                shift_vector[0] * conversion_factor_x, shift_vector[1] * conversion_factor_y)

            aim_x = origin_position_x - shift_vector_device[0]
            ...
            aim_y = origin_position_y + shift_vector_device[1]
            ...
            current_error_x = get_current_position(self.device_profile["servo"][1], "servo", None) - aim_x
            current_error_y = get_current_position(self.device_profile["servo"][2], "servo", None) - aim_y
            ...
            threshold_error = 1  # Some small value to determine when to stop adjusting

            while error_x > threshold_error or error_y > threshold_error:
                # Get the current error
                current_error_x = get_current_position(self.device_profile["servo"][1], "servo", None) - aim_x
                current_error_y = get_current_position(self.device_profile["servo"][2], "servo", None) - aim_y
                print(get_current_position(self.device_profile["servo"][1], "servo", None), aim_x)
                print(get_current_position(self.device_profile["servo"][2], "servo", None), aim_y)

                output_x = current_error_x  # Assuming desired shift is 0 (i.e., perfectly aligned)
                output_y = current_error_y
                ...
                print(move_CCdevice(self.device_profile["servo"][2], "servo", None, m_direction_y, abs(round(output_y)),
                                    velocity_y))
            #
            #     # Calculate the PID output for corrections
            #     output_x = int(current_error_x)  # Assuming desired shift is 0 (i.e., perfectly aligned)
            #     output_y = -int(current_error_y)
            #
            #     if output_x >= 0:
            #         m_direction_x = False
            #     else:
            #         m_direction_x = True
            #     print(move_CCdevice(self.device_profile["servo"][1], "servo", None, m_direction_x, abs(output_x),
            #                         velocity_x))
            #
            #     if output_y >= 0:
            #         m_direction_y = False
            #     else:
            #         m_direction_y = True
            #     print(move_CCdevice(self.device_profile["servo"][2], "servo", None, m_direction_y, abs(output_y),
            #                         velocity_y))
            #
