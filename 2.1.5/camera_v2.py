...
def adjust_rgb_offset(val=None):
    global r_offset, g_offset, b_offset
...
def main():
    ...
    cv2.createTrackbar('R offset', 'Camera', 0, 255, adjust_rgb_offset)
    cv2.createTrackbar('G offset', 'Camera', 0, 255, adjust_rgb_offset)
    cv2.createTrackbar('B offset', 'Camera', 0, 255, adjust_rgb_offset)
    ...
    while True:
       ...
       # Check and correct if zoomed region exceeds frame boundaries
        ...
        zoomed_region = frame[top:top+zoomed_height, left:left+zoomed_width]
        ...
        frame[..., 0] = cv2.add(frame[..., 0], b_offset)  # Blue channel
        frame[..., 1] = cv2.add(frame[..., 1], g_offset)  # Green channel
        frame[..., 2] = cv2.add(frame[..., 2], r_offset)  # Red channel
